...
# creo una nuova colonna per i gruppi forestali: utilizzo il valore del gruppo del secondo dataframe, se Ã¨ NA uso quello del primo
...
# 2 - metto a 0 i vari valori di carbonio per le resource units con profonditÃ  == 0
for(i in 1:nrow(ru_infc)){
...
for(i in 1:nrow(infcv)){
  ...
  # Lh Ã¨ presente solo in due resource units.Do 0 a i valori di necromassa fine e suolo
...
# inserisco valore SOMC per bodenlands MW che avevo dimenticato precedentemente. Il valore Ã¨ calcolato mediando quelli delle altre
